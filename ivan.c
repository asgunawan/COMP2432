    ...
    while (1) {
        ...
        if (!isBatchFile && strncmp(line, "addBatch", 8) == 0) {
            ...
            if (!validateCommandFormat(line, cmdType)) {
                printf("Error: Invalid command format123\n");
                ...
                switch (cmdType) {
                    ...
                    case CMD_ADD_RESERVATION:
                        printf("addReservation -member_X YYYY-MM-DD HH:MM n.n battery cable;\n");
                        break;
                    case CMD_ADD_EVENT:
                        printf("addEvent -member_X YYYY-MM-DD HH:MM n.n umbrella valetpark facility3;\n");
                        break;
